...
// ─── 사주 기둥 계산 ─────────────────────────────────
function calculatePillars(birthDate: string, birthTime: string) {
...
export function preprocessLocally(data: { saju?: { birthDate: string; birthTime: string; isLunar: boolean; gender: string }; mbti?: string; faceImage?: string; palmImage?: string }): PreprocessedData {
  const saju = data.saju || { birthDate: '2000-01-01', birthTime: '12:00', isLunar: false, gender: 'MALE' };
  
  const pillars = calculatePillars(saju.birthDate, saju.birthTime);
